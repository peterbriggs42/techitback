	...
	WelcomeSections.model.find()
		...
		WelcomeCauses.model.find()
			...
			view.render('home/welcome_causes_selected', {
				...
				// Causes
				previous: 		causes.filter(common.getItemByKey, "stopbullying")[0],
				selected: 		causes.filter(common.getItemByKey, "thrive")[0],
				next: 			causes.filter(common.getItemByKey, "memorial")[0],
			});
		});
	});
};