	...
	WelcomeSections.model.find()
		...
		WelcomeCauses.model.find()
			...
			view.render('home/welcome_causes_selected', {
				...
				// Causes
				previous: 		causes.filter(common.getItemByKey, "healthy")[0],
				selected: 		causes.filter(common.getItemByKey, "stomp")[0],
				next: 			causes.filter(common.getItemByKey, "moodoff")[0],
			});
		});
	});
};