	...
	WelcomeSections.model.find()
		...
		WelcomeCauses.model.find()
			...
			view.render('home/welcome_causes_selected', {
				...
				// Causes
				previous: 		causes.filter(common.getItemByKey, "headspace")[0],
				selected: 		causes.filter(common.getItemByKey, "tyler")[0],
				next: 			causes.filter(common.getItemByKey, "megan")[0],
			});
		});
	});
};